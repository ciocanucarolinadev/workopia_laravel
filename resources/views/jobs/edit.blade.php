...
>
    ...
    @method('PUT')
        ...
        <x-inputs.text-area id="description" name="description" label="Description" placeholder="We are seeking a skilled and motivated Software Developer to join our growing development team..." :value="old('description', $job->description)" />
        ...
        <x-inputs.select id="job_type" name="job_type" label="Job Type" value="{{old('job_type')}}" :options="[
            ...
            'Temporary' => 'Temporary',
            'Volunteer' => 'Volunteer',
            'On-Call' => 'On-Call']" :value="old('job_type', $job->job_type)" />

        <x-inputs.select id="remote" name="remote" label="Remote" value="{{old('remote')}}" :options="[0 => 'No', 1 => 'Yes']" :value="old('remote', $job->remote)" />
        ...
        <x-inputs.text id='company_website' type='url' name='company_website' label="Company Website" placeholder="Enter Company Website" :value="old('company_website', $job->company_website)" />

        <x-inputs.text id='contact_phone' name='contact_phone' label="Contact Phone" placeholder="Enter Contact Phone" :value="old('contact_phone', $job->contact_phone)" />

        <x-inputs.text id='contact_email' name='contact_email' type='email' label="Contact Email" placeholder="Enter Contact Email" :value="old('contact_email', $job->contact_email)" />
